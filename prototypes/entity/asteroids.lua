        ...
        {
            {
                ...
                localized_name = {"parameter-x", tostring(number)},
                subgroup = "parameters",
                order = "a",
                parameter = true
            }
...
local shared_resistances = {
    ...
    laser = {
        decrease = {0, 0, 0, 0, 0},
        percent = {0, 20, 90, 95, 95}
    },
}

local crystalite_resistances = {
    physical = {
        decrease = {0, 0, 0, 2000, 3500},
        percent = {0, 0, 15, 15, 15}
    },
    explosion = {
        decrease = {0, 0, 0, 0, 0},
        percent = {0, 35, 20, 10, 95}
    },
    laser = {
        decrease = {0, 0, 0, 0, 0},
        percent = {0, 50, 85, 90, 95}
    },
}

local shared_health = {0, 100, 400, 2000, 5000}
local shared_mass = {0, 200000, 500000, 5000000, 100000000}
local crystalite_health = {0, 125, 500, 2500, 6000}

local asteroid_data = {
    astralite = {
        order = "z",
        mass = shared_mass,
        max_health = crystalite_health,
        resistances = crystalite_resistances,
        shading_data = {
            normal_strength = 1.2,
            light_width = 0,
            brightness = 0.9,
...
local function get_graphic(asteroid_type, size, layer, suffix)
    return "__dark-isotope__/graphics/entity/asteroid/" .. asteroid_type .. "/" .. asteroid_sizes[size] .. "/asteroid-" .. asteroid_type .. "-" .. asteroid_sizes[size] .. "-" .. layer .. "-" .. suffix .. ".png"
end

local function asteroid_variation(asteroid_type, suffix, scale, size)
    return {
        color_texture = {
            filename = get_graphic(asteroid_type, size, "colour", suffix),
            size = sizes_resolution[size][1],
            scale = scale
        },
        shadow_shift = {0.25 * size , 0.25 * size},
        normal_map = {
            filename = get_graphic(asteroid_type, size, "normal", suffix),
            premul_alpha = false,
            size = sizes_resolution[size][1],
            scale = scale
        },
        roughness_map = {
            filename = get_graphic(asteroid_type, size, "roughness", suffix),
            premul_alpha = false,
            size = sizes_resolution[size][1],
            scale = scale
        }
...
for asteroid_size, asteroid_size_name in pairs(asteroid_sizes) do
    for asteroid_type, asteroid_data in pairs(asteroid_data) do
        local graphics_scale = graphics_scale[asteroid_size]
        local collision_radius = collision_radius[asteroid_size]
        ...
        data:extend{
            {
                ...
                localized_description = {"entity-description." .. asteroid_type .. "-asteroid"},
                icon = "__dark-isotope__/graphics/icons/" .. asteroid_name .. ".png",
                icon_size = 64,
                ...
                graphics_set = asteroid_graphics_set(0.0003 * (6 - asteroid_size), asteroid_data.shading_data, variations),
